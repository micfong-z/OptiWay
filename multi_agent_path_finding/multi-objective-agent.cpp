...
using PathPQ =
	priority_queue<StudentPath, vector<StudentPath>, CompareStudentPath>;
...
json getRoutesfromTimetable(const json &timetables, const Graph &graph,
	...
	for (const auto &[student, timetable] : timetables.items()) {
		...
		for (const auto &[day, classes] : timetable.items()) {
			...
			// Start of the day, going from G_floor to the first
			// class
			string path_str = shortest_paths["G" + classes["1"].get<string>()];
			today["0"] = path_str;

			// Lunch time is denoted by period "6", and periods
			// after are delayed by one index
			path_str = shortest_paths
				["G" + classes["7"].get<string>()]; // Originally, P7 is the
			...
			for (int period = 1; period <= 11; period++) {
				...
				if (period == 7) {
					offset = 1; // Add offset to be minused
								// because P7 is now P6
					continue;
				}
				...
				if (period != 11) {
					// If AS/AL students have P6s, the
					// should go to G_floor in the end
					...
					if (next_room[0] != 'A' && next_room[0] != 'B' &&
						next_room[0] != 'G')
						continue;
					...
					if (next_room == current_room)
						...
						string path_str =
							shortest_paths[current_room + next_room];
...
string concatenate(const vector<string> &vec) {
	...
	for (const auto &str : vec) {
		if (!result.empty()) {
			result += " "; // Add space before appending, but not
...
// Code to get the shortest path from start to end using Dijkstra's algorithm
// with congestion penalty
vector<string>
getDijkstraPenaltiedPath(const string &start, const string &end,
						 const Graph &graph,
	...
	while (!pq.empty()) {
		...
		try {
			for (const auto &edge : graph.at(current)) {
				...
				double edgeWeight =
					edge.weight +
					(CONGESTION_PENALTY * congestion.at(current + edge.dest));
				if (!visited[edge.dest] &&
					...
					prev[edge.dest] = current; // save the previous node
	...
	reverse(path.begin(),
			path.end()); // since the path is constructed in reverse
...
double computePerformanceIndex(const vector<string> &route,
	...
	for (int i = 0; i < route.size() - 1; i++) {
		...
		if (start == "G" || end == "G")
			continue; // Ignore the paths to and from G_floor
...
void iter(PathPQ &paths, double &sum_rperf,
		  unordered_map<string, int> &congestion, const Graph &graph,
		  string &last_start, string &last_end, vector<StudentPath> &temp) {
	// A single iteration
	StudentPath worst_path;
	bool flag = false;
	while (!paths.empty()) {
		worst_path = paths.top(); // The student whose route has the
								  // greatest r_perf
		paths.pop();
	...
	if (new_rperf < worst_path.rperf) {
		...
		// if the path doesn't change, the student's path cannot be
		// further optimized as the congestion penalty is already too
		// high
		temp.push_back(worst_path);
		sum_rperf += worst_path.rperf;
...
void iterMultiple(PathPQ &paths, double &sum_rperf,
	...
	for (int i = ITER_COUNT; i < ITER_NUM; i++) {
		...
		if (i % BATCH_SIZE | (i == 0))
			...
			vector<StudentPath> all_paths; // contain all the paths from paths
										   // pq and temp vector
			PathPQ new_paths;
			...
			for (const StudentPath &path : all_paths) {
				double rperf =
					computePerformanceIndex(path.path, congestion, graph);
				sum_rperf += rperf;
			}
...
void iterSinglePeriod(const int day, const int period, json &route_tables,
					  const Graph &graph) {
	// route_tables is the floyd_warshall's routes for all students at all
	// periods generated from gerRoutesfromTimetable() Initialize the
	// congestion matrix
	...
	for (const auto &[student, route_table] : route_tables.items()) {
		vector<string> route = vectorizeString(
			route_table[to_string(day)][to_string(period)].get<string>());
		for (int i = 0; i < route.size() - 1; i++) {
			congestion[route[i] + route[i + 1]]++;
	...
	while (!paths.empty()) {
		StudentPath path = paths.top();
		paths.pop();
		route_tables[path.id][to_string(day)][to_string(period)] =
...
// Code for generating the route for a single day, with each iter covering all
// periods
void iterSingleDay(const int day, json &route_tables, const Graph &graph,
	...
	for (int period = 0; period <= 11; period++) {
		if (period == 1 || period == 3 || period == 8 || period == 10)
			continue; // All the students are having classes right
					  // now
		...
		for (const auto &[student, route_table] : route_tables.items()) {
			const string route_str =
				route_table[to_string(day)][to_string(period)];
		...
		for (const auto &[student, route_table] : route_tables.items()) {
			...
			vector<string> route;
			if (route_str == "")
				route = {"G", "G"}; // if this student is having
									// a spare class
			else
				route = vectorizeString(route_str);
			double rperf =
				computePerformanceIndex(route, congestions[period], graph);
			StudentPath path = {student, rperf, route};
		...
		// cout << "PERIOD " << period << " INITIAL PERFORMANCE: " <<
		// sum_rperf[period] << endl;
	}
	...
	for (int i = ITER_COUNT + 1; i <= ITER_NUM; i++) {
		for (int period = 0; period <= 11; period++) {
			if (period == 1 || period == 3 || period == 8 || period == 10)
				continue; // All the students are having classes
						  // right now

			iter(paths[period], sum_rperf[period], congestions[period], graph,
				 last_start[period], last_end[period], temp[period]);
			prev_best_rperf[period] =
				min(sum_rperf[period], prev_best_rperf[period]);

			if (i % BATCH_SIZE | (i == 0)) {
				// cout << fixed << setprecision(0) << "0 " << i
				// << ' ' << day << ' ' << period << ' ' <<
				// sum_rperf[period] << ' ' <<
				...
				vector<StudentPath> all_paths; // contain all the paths from
											   // paths pq and temp vector
				PathPQ new_paths;
				// Get the congestion for the period
				...
				for (const StudentPath &path : all_paths) {
					double rperf = computePerformanceIndex(
						path.path, congestions[period], graph);
					sum_rperf[period] += rperf;
				}
				...
				prev_best_rperf[period] =
					min(sum_rperf[period], prev_best_rperf[period]);
				perf_indices[to_string(day)][to_string(period)] =
					int(sum_rperf[period]);
			...
			if (!(i % ITER_SAVE_STEPS) && period == 0 &&
				...
				vector<pair<string, string>>
					all_paths; // contain the students and their
							   // related paths
				...
				for (const auto &path : all_paths)
					route_tables[path.first][to_string(day)]
								[to_string(period)] = path.second;
				...
				ofstream iter_output_file(ROUTE_FILE_PATH + "_" +
										  to_string(day) + ".json");
				if (iter_output_file.is_open()) {
					iter_output_file << iter_output.dump(); // minimize size
					iter_output_file.close();
					// cout << "JSON data written to
					// routes.json successfully." << endl;
					cout << fixed << setprecision(0) << "1 " << i << ' ' << day
						 << ' ' << period << ' ' << sum_rperf[period] << ' '
						 << prev_best_rperf[period] << endl;
				} else {
					// cerr << "Failed to open the file for
					// writing." << endl;
...
int main(int argc, char **argv) {
	...
	for (int i = 0; i < argc; ++i) {
		if (strcmp(argv[i], "-b") == 0 &&
			i + 1 < argc) { // Batch Size: the amount of iterations
							// before updating the congestion
		...
		if (strcmp(argv[i], "-s") == 0 &&
			i + 1 < argc) { // Save Iteration Step: the amount of
							// iterations to before each save
