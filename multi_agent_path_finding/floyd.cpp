...
void initializeDistanceAndPredecessorMatrices(const Graph &graph,
											  DistanceMatrix &dist,
...
void floydWarshall(const Graph &graph, DistanceMatrix &dist,
				   PredecessorMatrix &pred) {
...
vector<string> reconstructPath(const PredecessorMatrix &pred,
...
Json getRoutesfromTimetable(const Json &timetables, const Graph &graph,
	...
	for (const auto &[student, timetable] : timetables.items()) {
		...
		for (const auto &[day, classes] : timetable.items()) {
			...
			path_str =
				shortest_paths["G" +
							   classes["7"].get<string>()]; // Originally, P7
															// is the first
															// afternoon class
			today["7"] = path_str;
...
// pre-calculated all the shortest paths in advance to avoid run-time
// calculations
...
void generate_all_floyd_distances(const Graph &graph) {
	...
	if (file.is_open()) {
		...
		file.close();
		std::cout << "JSON data written to distances.json successfully."
				  << endl;
	} else {
		std::cerr << "Failed to open the file for writing." << endl;
	}
